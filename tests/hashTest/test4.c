/* 
 * test4.c --- search for each entry in the hash table
 * 
...
int main(void) {
	...
	for (cnt = 0; cnt < 100; cnt++) {
		if ((src = (int *)hsearch(h, searchFunc, (const char *)&cnt, sizeof(cnt))) == NULL) {
			fprintf(stderr, "Could not find entry in table.\n");
			exit(EXIT_FAILURE);
		}
		
		if (cnt != *src) {
			fprintf(stderr, "Did not return correct entry.\n");
			exit(EXIT_FAILURE);
		}
	}
